...
def generate_markdown_report_v2(
    ...
    """
    生成带追溯关系的Markdown格式论文审核报告（新版）

    Args:
        full_audit_data: 完整审核数据（包含一_主要问题分析、二_方法创新性、三_评估验证）
        output_dir: 输出目录
...
"""
    ...
    # ========== 一、主要问题分析 ==========
    main_problems = full_audit_data.get('一_主要问题分析', [])
    md += f"""
...
> **分析说明：** 现有方法在什么情况下解决不好或解决不了，揭示方法层面的根本原因

| 问题编号 | 问题描述 | 现有方法表现 | 问题根源 | 对应创新点 |
|----------|----------|--------------|----------|------------|
"""
    ...
    if main_problems:
        for p in main_problems:
            ...
            linked_innovations = p.get('对应创新点', [])
            linked_str = ', '.join(linked_innovations) if linked_innovations else '未对应'
            md += f"| {problem_id} | {problem_desc} | {existing_perf} | {root_cause} | {linked_str} |\n"
    else:
        md += "| 暂无数据 | - | - | - | - |\n"

    # ========== 二、方法创新性 ==========
    method_innovations = full_audit_data.get('二_方法创新性', full_audit_data.get('method_innovations', []))
    md += f"""
...
> **分析说明：** 本文提出什么方法，如何在方法层面解决已有问题（追溯到问题）

| 创新编号 | 创新点 | 方法章节 | 创新类型 | 解决的问题 | 状态 |
|----------|--------|----------|----------|------------|------|
"""
    ...
    if method_innovations:
        for m in method_innovations:
            ...
            linked_str = ', '.join(linked_problems) if linked_problems else '未对应'
            status = m.get('status', '✅')
            md += f"| {innovation_id} | {innovation} | {section} | {innovation_type} | {linked_str} | {status} |\n"
    else:
        md += "| 暂无数据 | - | - | - | - | - |\n"

    # ========== 三、评估验证 ==========
    evaluation = full_audit_data.get('三_评估验证', full_audit_data.get('evaluation', {}))
    md += f"""
...
> **评估说明：** 研究问题、数据集、评价指标、评估结果、局限性（追溯到创新点）
...
"""
    ...
    rqs = evaluation.get('research_questions', [])
    if rqs:
        for rq in rqs:
            rq_id = rq.get('rq_id', '')
            rq_desc = rq.get('rq描述', '')
...
"""
    ...
    if datasets:
        for ds in datasets:
            if isinstance(ds, dict):
                md += f"| {ds.get('dataset_name', '')} | {ds.get('size', '')} | {ds.get('description', '')} |\n"
            else:
                md += f"| {ds} | - | - |\n"
    else:
        md += "| 暂无数据 | - | - |\n"
...
"""
    ...
    if metrics:
        for mt in metrics:
            if isinstance(mt, dict):
                higher = '↑' if mt.get('higher_better', True) else '↓'
                md += f"| {mt.get('metric_name', '')} | {mt.get('definition', '')} | {higher} |\n"
            else:
                md += f"| {mt} | - | - |\n"
    else:
        md += "| 暂无数据 | - | - |\n"
...
"""
    ...
    if results:
        main_results = results.get('主实验结果', {})
        if main_results:
            md += f"| 指标 | 数值 |\n|------|------|\n"
...
"""
    ...
    if limitations:
        for lim in limitations:
            lim_text = lim.get('limitation', lim) if isinstance(lim, dict) else lim
            acknowledged = '✅ 论文已承认' if (isinstance(lim, dict) and lim.get('acknowledged_by_paper', False)) else ''
            md += f"- {lim_text} {acknowledged}\n"
    else:
        md += "- 暂无局限性数据\n"

    # ========== 四、方法 vs Baseline详细对比 ==========
    md += f"""

---

## 四、方法 vs Baseline 详细对比
...
"""
            ...
            md += f"\n**性能指标:**\n"
            if metrics_list:
                md += f"| 指标 | Baseline值 | 本文方法 | 提升 |\n|------|-----------|----------|------|\n"
                for m in metrics_list:
                    metric = m.get('metric', '')
                    baseline_val = m.get('baseline_value', '-')
                    proposed_val = m.get('proposed_value', '-')
                    improvement = m.get('improvement', '-')
                    md += f"| {metric} | {baseline_val} | {proposed_val} | {improvement} |\n"
            else:
                md += "- 未提供\n"
...
def generate_english_html_report(
    full_audit_data: Dict = None,
    output_dir: str = "."
) -> str:
    """
    ...
    paper_title = full_audit_data.get('paper_title', 'Paper Content Audit Report')
    basic_info = full_audit_data.get('basic_info', {})
    ...
    for key, value in basic_info.items():
        en_key = en_labels.get(key, key)
        basic_info_rows += f"      <tr><td class='label'>{en_key}</td><td>{value}</td></tr>\n"
    ...
    for p in main_problems:
        ...
        root_cause = p.get('问题根源', '')
        linked_innovations = p.get('对应创新点', [])
        linked_str = ', '.join(linked_innovations) if linked_innovations else 'Not linked'
    ...
    for m in method_innovations:
        ...
        status = m.get('status', '✅')
        linked_problems = m.get('解决的问题', m.get('linked_problems', []))
        innovation_type = m.get('创新类型', 'To be confirmed')
        linked_str = ', '.join(linked_problems) if linked_problems else 'Not linked'
        status_class = 'pass' if '✅' in status else ('fail' if '❌' in status else 'warning')
        innovations_rows += f"""      <tr>
        <td class='innovation-id'>{innovation_id}</td>
        <td>{innovation}</td>
    ...
    for rq in evaluation.get('research_questions', []):
        ...
        rq_desc = rq.get('rq描述', '')
        linked = rq.get('对应的创新点', [])
        linked_str = ', '.join(linked) if linked else 'Not linked'
        rq_html += f"      <tr><td>{rq_id}</td><td>{rq_desc}</td><td>{linked_str}</td></tr>\n"
    ...
    for lim in limitations:
        lim_text = lim.get('limitation', lim) if isinstance(lim, dict) else lim
        acknowledged = '✅ Acknowledged' if (isinstance(lim, dict) and lim.get('acknowledged_by_paper', False)) else ''
        limitations_html += f"      <li>{lim_text} {acknowledged}</li>\n"
    ...
    for item in method_vs_baseline:
        improvements = item.get('improvements', [])
        metrics = item.get('metrics', [])
        improvements_li = "".join([f"          <li>{imp}</li>\n" for imp in improvements]) if improvements else "          <li class='none'>Not provided</li>\n"

        metrics_content = ""
        if metrics:
            for m in metrics:
                if isinstance(m, dict):
                    metrics_content += f"          <li>{m.get('metric', '')}: {m.get('baseline_value', '-')} → {m.get('proposed_value', '-')} ({m.get('improvement', '-')})</li>\n"
                else:
                    metrics_content += f"          <li>{m}</li>\n"
        else:
            metrics_content = "          <li class='none'>Not provided</li>\n"

        method_diff_html += f"""
        <div class="comparison-card">
          <h4>{item.get('method_name', 'Proposed Method')} vs {item.get('baseline', 'Baseline')}</h4>
          <table class='inner-table'>
            <tr><th>Dimension</th><th>Content</th></tr>
            <tr><td class='label'>Improvements</td><td><ul>{improvements_li}</ul></td></tr>
            <tr><td class='label'>Metrics</td><td><ul>{metrics_content}</ul></td></tr>
          </table>
        </div>
"""

    # Overall scores
    scores = full_audit_data.get('overall_scores', [])
    scores_grid = ""
    for s in scores:
        result = s.get('result', '')
        if '✅' in result:
...
"""
    ...
    trace_check = summary.get('traceability_check', {})
    trace_ok = trace_check.get('problems_with_solutions', False) and trace_check.get('solutions_with_evaluation', False)
    trace_msg = '✅ Traceability Complete' if trace_ok else '⚠️ Traceability Needs Improvement'
    trace_detail = f"Problem→Solution: {'✅' if trace_check.get('problems_with_solutions', False) else '❌'} | Solution→Evaluation: {'✅' if trace_check.get('solutions_with_evaluation', False) else '❌'} | Evaluation→Problem: {'✅' if trace_check.get('evaluation_traces_to_problems', False) else '❌'}"

    audit_date = basic_info.get('审核时间', timestamp.split('_')[0])
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>{paper_title} - Paper Content Audit Report</title>
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{
...
<div class="container">
  <div class="header">
    <h1>Paper Content Audit Report</h1>
    <p class="subtitle">{paper_title}</p>
    <p class="subtitle">Audit Date: {audit_date}</p>
  </div>
  ...
  <div class="section">
    ...
    <div class="trace-box">
      <div class="trace-diagram">
        <div class="trace-node problem">1. Main Problems</div>
        <div class="trace-arrow">→</div>
        <div class="trace-node innovation">2. Method Innovation</div>
        <div class="trace-arrow">→</div>
        <div class="trace-node evaluation">3. Evaluation</div>
      </div>
      <p><strong>{trace_msg}</strong></p>
      <p style="font-size:12px; color:#666;">{trace_detail}</p>
    </div>
  ...
  <div class="section">
    <h2>1. Main Problem Analysis</h2>
    <div class="warning-box">
      <strong>Analysis:</strong> Under what circumstances do existing methods perform poorly or cannot solve, revealing root causes at the method level
    </div>
    <table>
      <tr><th>Problem ID</th><th>Problem Description</th><th>Existing Method Performance</th><th>Root Cause (Method Level)</th><th>Linked Innovations</th></tr>
{problems_html if problems_html else '      <tr><td colspan="5">No problem analysis data available</td></tr>'}    </table>
  </div>

  <div class="section">
    <h2>2. Method Innovation</h2>
    <div class="info-box">
      <strong>Analysis:</strong> What methods does this paper propose, and how do they solve existing problems at the method level (traced to problems)
    </div>
    <table>
      <tr><th>Innovation ID</th><th>Innovation</th><th>Section</th><th>Details</th><th>Type</th><th>Problems Solved</th><th>Status</th></tr>
{innovations_rows if innovations_rows else '      <tr><td colspan="7">No innovation data available</td></tr>'}    </table>
  </div>

  <div class="section">
    <h2>3. Evaluation & Validation</h2>
    <div class="trace-box">
      <strong>Evaluation:</strong> Research questions, datasets, metrics, results, limitations (traced to innovation points)
    </div>

    <h3>Research Questions (RQ)</h3>
    <table>
      <tr><th>RQ ID</th><th>Research Question</th><th>Linked Innovations</th></tr>
{rq_html if rq_html else '      <tr><td colspan="3">No research questions data available</td></tr>'}    </table>

    <h3>Datasets</h3>
    <table>
      <tr><th>Dataset</th><th>Size</th><th>Description</th></tr>
{datasets_html if datasets_html else '      <tr><td colspan="3">No dataset data available</td></tr>'}    </table>

    <h3>Evaluation Metrics</h3>
    <table>
      <tr><th>Metric</th><th>Definition</th><th>Higher is Better</th></tr>
{metrics_html if metrics_html else '      <tr><td colspan="3">No metrics data available</td></tr>'}    </table>

    <h3>Key Findings</h3>
    <ul>
{findings_html if findings_html else '      <li>No key findings data available</li>'}    </ul>

    <h3>Limitations</h3>
    <ul>
{limitations_html if limitations_html else '      <li>No limitations data available</li>'}    </ul>
  </div>

  <div class="section">
    <h2>4. Method vs Baseline Detailed Comparison</h2>
    <div class="info-box">
      <strong>Evaluation:</strong> Specific improvements and performance metrics of each method relative to baseline
    </div>
{method_diff_html if method_diff_html else '    <p class="none">No detailed comparison data available</p>'}  </div>

  <div class="section">
    <h2>5. Overall Scores</h2>
    <div class="score-grid">
{scores_grid if scores_grid else '      <div class="score-item"><span>No scoring data available</span></div>'}    </div>
  </div>
  ...
  <div class="section">
    ...
    <h3>Strengths</h3>
    <ul class="strengths-list">
{strengths_li if strengths_li else '      <li>No clear strengths identified</li>'}    </ul>
    <h3>Weaknesses</h3>
    <ul class="weaknesses-list">
{weaknesses_li if weaknesses_li else '      <li>No significant weaknesses identified</li>'}    </ul>
  </div>

  <div class="footer">
    <p>Generated by Paper Content Audit Skill</p>
    ...
    en_html_path = os.path.join(output_dir, f"paper_audit_report_{timestamp}_en.html")
    with open(en_html_path, 'w', encoding='utf-8') as f:
        f.write(html)

    print(f"🌐 English HTML report saved: {en_html_path}")
    return en_html_path
...
def generate_all_reports(
    ...
    """
    生成所有格式的报告：Markdown、中文HTML、英文HTML
    ...
    md_path = generate_markdown_report_v2(full_audit_data, output_dir)
    if md_path:
        results['markdown'] = md_path

    # 2. Generate Chinese HTML report
    try:
        # First generate to a temp path, then rename
        temp_html_path = os.path.join(output_dir, f"temp_zh_{timestamp}.html")

        # Call the Chinese HTML generator with custom save
        html_content = _build_unified_html_report(full_audit_data, timestamp.split('_')[0])
        with open(temp_html_path, 'w', encoding='utf-8') as f:
            f.write(html_content)

        zh_path = os.path.join(output_dir, f"paper_audit_report_{timestamp}_zh.html")
        shutil.move(temp_html_path, zh_path)
        results['chinese_html'] = zh_path
        print(f"🌐 Chinese HTML report saved: {zh_path}")
    except Exception as e:
        print(f"Error generating Chinese HTML: {e}")

    # 3. Generate English HTML report
    try:
        en_html_path = generate_english_html_report(full_audit_data, output_dir)
        if en_html_path:
            results['english_html'] = en_html_path
    except Exception as e:
        print(f"Error generating English HTML: {e}")
