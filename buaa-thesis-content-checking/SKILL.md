...
description: |
  ...
  功能：使用Claude Code终端直接进行LLM分析和Agent生成报告。分析论文的主要贡献、方法创新性、实验评估，以及与baseline的详细对比，生成带追溯关系的中文Markdown和HTML格式的完整审核报告。
  工作流程：PDF文本提取 → LLM综合分析 → Agent生成报告（Markdown + HTML）
  重要更新：Task 4使用Claude Code Agent生成报告，分析现有方法问题时必须给出具体参考文献，并判断是否为近3年内工作（2023-2026）和顶会顶刊。
...
### Task 4: 报告生成（Claude Code Agent执行）

**创建任务**:
```
TaskCreate: "生成审核报告 - {论文文件名}"
描述: 使用Claude Code Agent读取audit_data JSON文件并生成Markdown和HTML格式的论文审核报告
```

**⚠️ 重要：此任务必须由Claude Code Agent执行，不是脚本执行**

**执行流程**:

1. **读取JSON数据文件**
   - Task 3保存的JSON文件位于：`audit_data_for_agent_YYYYMMDD_HHMMSS.json`
   - 或在当前工作目录查找最新的`audit_data_for_agent_*.json`文件

2. **Agent执行命令**:
   ```
   使用Agent工具启动"报告生成Agent"，读取JSON数据并生成3个报告文件
   ```
...
| 格式 | 文件名 | 内容要求 |
|------|--------|----------|
| Markdown | `paper_audit_report_YYYYMMDD_HHMMSS.md` | 中文Markdown格式，带追溯关系 |
| 中文HTML | `paper_audit_report_YYYYMMDD_HHMMSS_zh.html` | 中文HTML，CSS样式精良 |
| 英文HTML | `paper_audit_report_YYYYMMDD_HHMMSS_en.html` | 纯英文HTML，所有内容翻译 |

4. **Markdown报告格式（必须包含以下章节）**:
...
| 中文 | 英文 |
|------|------|
| 论文内容审核报告 | Paper Content Audit Report |
| 一、主要问题分析 | 1. Main Problem Analysis |
| 二、方法创新性 | 2. Method Innovation |
| 三、评估验证 | 3. Evaluation & Validation |
| 四、方法 vs Baseline详细对比 | 4. Method vs Baseline Comparison |
| 五、综合评分 | 5. Overall Scores |
| 六、总结 | 6. Summary |
| 追溯关系总览 | Traceability Overview |
...
| 现有方法表现 | Existing Method Performance |
| 问题根源 | Root Cause |
| 对应创新点 | Linked Innovations |
| 创新编号 | Innovation ID |
| 创新点 | Innovation |
| 方法章节 | Section |
...
| 格式 | 文件名 | 说明 |
|------|--------|------|
| Markdown报告 | `paper_audit_report_YYYYMMDD_HHMMSS.md` | 带追溯关系的完整审核报告（中文） |
| 中文HTML报告 | `paper_audit_report_YYYYMMDD_HHMMSS_zh.html` | 完整中文HTML审核报告 |
| 英文HTML报告 | `paper_audit_report_YYYYMMDD_HHMMSS_en.html` | 纯英文HTML审核报告 |
...
5. TaskUpdate - Task 3完成后，标记Task 4为进行中，启动Agent生成报告
6. Agent执行报告生成，生成Markdown报告、中文HTML报告、英文HTML报告
7. TaskUpdate - Task 4完成后，所有任务标记为已完成

8. TaskList - 展示最终审核状态汇总表

| ID | 审核阶段 | 状态 | 结果 |
|----|---------|------|------|
| #1 | PDF文本提取 | ✅ 完成 | 提取了82页文本 |
| #2 | 参考文献调研 | ✅ 完成 | 调研了10篇文献 |
| #3 | LLM综合分析 | ✅ 完成 | 生成结构化JSON |
| #4 | 报告生成 | ✅ 完成 | paper_audit_report_xxx.md + _zh.html + _en.html |

9. 启动Agent生成报告（Task 4的后续步骤）:
   Agent "报告生成Agent" 读取JSON数据，生成Markdown、中文HTML、英文HTML三个报告
```
