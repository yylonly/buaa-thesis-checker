...
from reports.generator import generate_html_report, generate_markdown_report_v2, generate_english_html_report, generate_all_reports
...
def run_step3(llm_data: dict, output_dir: str = "."):
    """Task 3: 保存审核数据，供后续Claude Code Agent生成报告使用"""
    print("\n" + "=" * 60)
    print("Task 3: 保存审核数据")
    print("=" * 60)
    ...
    # 保存审核数据到临时文件，供Claude Code Agent使用
    import json
    from datetime import datetime as dt
    timestamp = dt.now().strftime("%Y%m%d_%H%M%S")
    audit_data_path = os.path.join(output_dir, f"audit_data_for_agent_{timestamp}.json")
    with open(audit_data_path, 'w', encoding='utf-8') as f:
        json.dump(full_audit_data, f, ensure_ascii=False, indent=2)

    print(f"\n📊 审核数据已保存到: {audit_data_path}")
    print("\n" + "=" * 60)
    print("✅ 审核数据已准备就绪")
    print("=" * 60)
    print("\n下一步: 使用Claude Code Agent根据此数据生成报告")
    print("参见SKILL.md中Task 4的Agent执行说明")
    print("\nAgent应读取JSON文件并生成3个报告:")
    print("  - paper_audit_report_YYYYMMDD_HHMMSS.md (Markdown)")
    print("  - paper_audit_report_YYYYMMDD_HHMMSS_zh.html (中文HTML)")
    print("  - paper_audit_report_YYYYMMDD_HHMMSS_en.html (英文HTML)")
